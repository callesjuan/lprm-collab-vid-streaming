  ...
  try:
    ...
    while True:
      try:
        ...
        if client.stream is not None and client.stream['status'] == 'streaming':
          now = datetime.datetime.now()
          if (now - last_message).total_seconds() >= 60:
            group_jid = client.stream['group_jid'] + '@' + client.MUC_JID
            body = client.muc_nick + ": hello i'm dummy "
            msg = client.make_message(mto=group_jid, mbody=body, mtype='groupchat').send()
            last_message = datetime.datetime.now()
          else:
            time.sleep(1)
